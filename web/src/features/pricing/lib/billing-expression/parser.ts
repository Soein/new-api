...
import {
  ...
  TIME_FUNCTIONS,
  TOKEN_VARIABLES,
  V1_BILLING_FUNCTIONS,
  V2_BILLING_FUNCTIONS,
...
class BillingParser {
  ...
  private version: ExpressionVersion = 1
  ...
  constructor(private readonly source: string) {
    ...
    const versionMatch = source.match(/^\s*v(\d+):/)
    if (versionMatch) {
      const v = Number(versionMatch[1])
      if (v !== 1 && v !== 2) {
        throw new BillingExpressionError({
          code: 'unsupported',
          detail: versionMatch[0].trim(),
        })
      }
      this.version = v as ExpressionVersion
      this.cursor = versionMatch[0].length
    }
    this.current = this.next()
  }

  getVersion(): ExpressionVersion {
    return this.version
  }
  ...
  private primary(): ExpressionNode {
    ...
    const functions =
      this.version === 2 ? V2_BILLING_FUNCTIONS : V1_BILLING_FUNCTIONS
    ...
    const end = this.expect(')').end
    if (args.length !== functions[token.text]) {
      throw new BillingExpressionError({
        code: 'type',
        detail: token.text,
        position: token.start,
  ...
  visitExpression(ast, (node) => {
    ...
    if (node.kind === 'literal') {
      ...
      if (node.name === 'header') result = 'string'
        ...
        result = 'number'
      } else if (node.name === 'per_image') {
        requireType(node.args[0], 'number')
        result = 'number'
      } else if (node.name === 'rule') {
        requireType(node.args[0], 'string')
        requireType(node.args[1], 'boolean')
        requireType(node.args[2], 'number')
        result = 'number'
...
function usesRequestProbe(node: ExpressionNode): boolean {
  const { functions } = expressionDependencies(node)
  return [...functions].some((name) =>
...
function containsTokenVar(node: ExpressionNode): boolean {
  const { variables } = expressionDependencies(node)
  return TOKEN_VARIABLES.some((tokenVar) => variables.has(tokenVar))
}

function isRequestPriceMultiplier(node: ExpressionNode): boolean {
  if (node.kind === 'variable' && node.name === 'image_count') {
    return true
  }
  if (node.kind === 'conditional') {
    if (
      !usesRequestProbe(node.condition) ||
      containsPricingMarker(node) ||
      containsTokenVar(node)
    ) {
      return false
    }
    return (
      ...
      Number.isFinite(node.yes.value) &&
      node.yes.value >= 0 &&
      node.no.kind === 'literal' &&
      node.no.value === 1
    )
  }
  if (node.kind === 'call' && node.name === 'rule' && node.args.length === 3) {
    ...
    const condNode = node.args[1]
    if (
      !usesRequestProbe(condNode) ||
      containsPricingMarker(node) ||
      containsTokenVar(node)
    ) {
      return false
    }
    const multNode = node.args[2]
    return (
      multNode.kind === 'literal' &&
      typeof multNode.value === 'number' &&
      Number.isFinite(multNode.value) &&
      multNode.value > 0
    )
  }
  return false
...
function validateFixedPricingTree(node: ExpressionNode): void {
  if (node.kind === 'conditional') {
    if (containsPricingMarker(node.condition)) {
      throw new BillingExpressionError({
        code: 'type',
        detail:
    ...
    validateFixedPricingTree(node.yes)
    validateFixedPricingTree(node.no)
    return
  }
  if (node.kind === 'binary' && node.operator === '*') {
    if (isRequestPriceMultiplier(node.right)) {
      validateFixedPricingTree(node.left)
      return
    }
    if (isRequestPriceMultiplier(node.left)) {
      validateFixedPricingTree(node.right)
      return
    }
  }
  if (
...
export function compileBillingExpression(source: string): CompilationResult {
  ...
  try {
    const parser = new BillingParser(source)
    ...
    visitExpression(ast, (node) => {
      ...
      } else if (node.kind === 'call' && node.name === 'rule') {
        const nameArg = node.args[0]
        const condArg = node.args[1]
        const multArg = node.args[2]
        let name: string | undefined
        if (nameArg.kind === 'literal' && typeof nameArg.value === 'string') {
          name = nameArg.value.trim().slice(0, 256)
        }
        if (
          multArg.kind === 'literal' &&
          typeof multArg.value === 'number' &&
          Number.isFinite(multArg.value) &&
          multArg.value > 0
        ) {
          requestRules.push({
            node,
            condition: condArg,
            multiplier: multArg.value,
            cond: source.slice(condArg.start, condArg.end),
            ...(name !== undefined ? { name } : {}),
          })
        }
      }
