...
func containsTokenVar(node ast.Node) bool {
	return ast.Find(node, func(part ast.Node) bool {
		identifier, ok := part.(*ast.IdentifierNode)
		if !ok {
			return false
...
func isRequestPriceMultiplier(node ast.Node) bool {
	...
	if conditional, ok := node.(*ast.ConditionalNode); ok {
		if !conditional.Ternary || !usesRequestProbe(conditional.Cond) || containsPricingMarker(conditional) || containsTokenVar(conditional) {
			return false
		}
	...
	if call, ok := node.(*ast.CallNode); ok {
		callee, isIdent := call.Callee.(*ast.IdentifierNode)
		if !isIdent || callee.Value != "rule" || len(call.Arguments) != 3 {
			return false
		}
		nameNode, isString := call.Arguments[0].(*ast.StringNode)
		if !isString || strings.TrimSpace(nameNode.Value) == "" {
			return false
		}
		condNode := call.Arguments[1]
		if !usesRequestProbe(condNode) || containsPricingMarker(call) || containsTokenVar(call) {
			return false
		}
		multiplier, multiplierOK := requestRuleNumber(call.Arguments[2])
		return multiplierOK && multiplier > 0 && !math.IsNaN(multiplier) && !math.IsInf(multiplier, 0)
	}
	return false
}
