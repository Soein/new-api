...
func setupLoginAtAuthVersion(user *model.User, expectedAuthVersion int64, c *gin.Context) {
	...
	// Keep the account-incarnation fence used by destructive administration
	// operations while dashboard authentication moves to versioned sessions.
	if err := model.EnsureUserSessionGeneration(user); err != nil {
		common.SysLog(fmt.Sprintf("failed to establish user identity generation for user %d: %v", user.Id, err))
		common.ApiErrorI18n(c, i18n.MsgDatabaseError)
		return
	}
...
func SearchUsers(c *gin.Context) {
	...
	pageInfo := common.GetPageQuery(c)
	sortOptions := model.NewUserSortOptions(c.Query("sort_by"), c.Query("sort_order"))
	users, total, err := model.SearchUsers(keyword, group, role, status, quotaOperator, quotaValue, pageInfo.GetStartIdx(), pageInfo.GetPageSize(), sortOptions)
	if err != nil {
		common.ApiError(c, err)
		return
	}
