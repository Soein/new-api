...
// RevokeUserAccessToken revokes a PAT only for the expected live account identity.
// It returns the revoked token fingerprint, or an empty string when no token exists.
func RevokeUserAccessToken(id int, expectedGeneration string) (string, error) {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		var user User
		if err := lockForUpdate(tx).Select("id", "access_token").
			Where("id = ? AND session_generation = ?", id, expectedGeneration).First(&user).Error; err != nil {
		...
		return tx.Model(&User{}).Where("id = ? AND session_generation = ?", id, expectedGeneration).
			Updates(map[string]interface{}{"access_token": nil, "access_token_created_at": nil}).Error
	})
	return tokenRef, err
}
...
func SearchUsers(keyword string, group string, role *int, status *int, quotaOperator string, quotaValue *int, startIdx int, num int, sortOptions ...UserSortOptions) ([]*User, int64, error) {
	var users []*User
	var total int64
	var err error
...
// GetUserSessionGeneration returns the stable account-incarnation identifier
// used to reject administrative actions issued against a reused numeric ID.
func GetUserSessionGeneration(id int) (string, error) {
	if id <= 0 {
		return "", ErrUserNotFound
	}
...
func GetSelfUserById(id int) (*User, error) {
	...
	err := DB.Model(&User{}).Select([]string{
		...
		"stripe_customer", "auth_version", "session_generation",
...
func HardDeleteUsersByIds(targets []UserDeletionTarget, actorId int, actorGeneration string) ([]*User, error) {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		for _, authenticationData := range []any{
			...
			if err := tx.Unscoped().Where("user_id IN ?", ids).Delete(authenticationData).Error; err != nil {
				return err
			}
		}
