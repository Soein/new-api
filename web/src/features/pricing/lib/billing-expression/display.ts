...
function tokenTier(
  ...
  if (
    ...
    return {
      ...
      conditions,
      prices,
      perImagePrice: body.args[0].value,
    }
  }
  let perImagePrice: number | undefined
  for (const term of flattenBinary(node.args[1], '+')) {
    if (
      term.kind === 'call' &&
      term.name === 'per_image' &&
      term.args[0]?.kind === 'literal' &&
      typeof term.args[0].value === 'number'
    ) {
      if (perImagePrice !== undefined) return null
      perImagePrice = term.args[0].value
      continue
    }
  ...
  return {
    ...
    conditions,
    prices,
    ...(perImagePrice !== undefined ? { perImagePrice } : {}),
  }
}
...
export function readTokenTierChain(node: ExpressionNode): TokenTier[] | null {
  if (
    ...
    node.name === 'per_image' &&
    node.args[0]?.kind === 'literal' &&
    typeof node.args[0].value === 'number'
  ) {
    return [
      {
        label: 'base',
        conditions: [],
        prices: {},
        perImagePrice: node.args[0].value,
      },
    ]
  }
  if (
