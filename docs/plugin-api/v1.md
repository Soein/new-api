...
`buildBatchQueryRequest(ctx, taskIds)` keeps the v1 second argument as an array of upstream task ID strings. Both batch hooks receive the complete `TaskQueryContext[]` through `ctx.tasks`, including each task's persisted `data` and `state`. `parseTaskResult` / `parseBatchResult` also receive `{status, headers}` for the upstream HTTP response.
