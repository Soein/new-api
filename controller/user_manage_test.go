...
func setupManageUserTestDB(t *testing.T) *gorm.DB {
	...
	require.NoError(t, db.AutoMigrate(&model.User{}, &model.UserQuotaDebt{}, &model.UserSession{}, &model.CasbinRule{}, &model.AuthzRole{}))
...
func TestManageUserQuotaTargetsAndWalletBounds(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "subtract_overdraw", mode: "subtract", before: 100, value: 101, targetID: 1, reason: "quota_insufficient"},
		{name: "subtract_from_debt", mode: "subtract", before: -100, value: 1, targetID: 1, reason: "quota_insufficient"},
...
func TestManageUserQuotaConcurrentSnapshots(t *testing.T) {
	...
	for range 2 {
		result := <-results
		require.NoError(t, result.err, "the mutation gate must serialize same-user adjustments on every database")
...
func TestManageUserQuotaCacheUsesCommittedIntegerDifference(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "add_repays_debt_first", mode: "add", before: -100, cached: -120, debt: 300, value: 500, after: 100, wantCached: 80},
		{name: "add_only_repays_debt", mode: "add", before: -100, cached: -120, debt: 600, value: 500, after: -100, wantCached: -120, wantDebt: 100},
		{name: "debt_repayment_at_wallet_limit", mode: "add", before: common.MaxWalletQuota, cached: common.MaxWalletQuota - 100, debt: 500, value: 500, after: common.MaxWalletQuota, wantCached: common.MaxWalletQuota - 100},
		...
		{name: "unchanged_override_clears_debt", mode: "override", before: -100, cached: -120, debt: 300, value: -100, after: -100, wantCached: -120},
		...
		{name: "failed_credit_restores_debt", mode: "add", before: -100, cached: -120, debt: 300, value: 500, after: -100, wantCached: -120, wantDebt: 300, failUpdate: true},
		{name: "failed_override_restores_debt", mode: "override", before: -100, cached: -120, debt: 300, value: 100, after: -100, wantCached: -120, wantDebt: 300, failUpdate: true},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, db.Create(&user).Error)
			if tc.debt > 0 {
				require.NoError(t, db.Create(&model.UserQuotaDebt{UserId: user.Id, Amount: tc.debt}).Error)
			}
			...
			debt, err := model.GetUserQuotaDebt(user.Id)
			require.NoError(t, err)
			assert.Equal(t, tc.wantDebt, debt)
			if !tc.failUpdate {
				var audit model.AuditLog
				require.NoError(t, model.LOG_DB.First(&audit).Error)
				params, err := common.Marshal(audit.Other.Op.Params)
				require.NoError(t, err)
				var snapshot struct {
					Before int `json:"from"`
					After  int `json:"to"`
				}
				require.NoError(t, common.Unmarshal(params, &snapshot))
				assert.Equal(t, tc.before, snapshot.Before)
				assert.Equal(t, tc.after, snapshot.After)
			}
			if tc.missingCache {
