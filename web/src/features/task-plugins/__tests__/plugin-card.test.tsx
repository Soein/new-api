...
test('marketplace entry for factory shows system update badge and does not offer installation', () => {
  ...
  render(
    <MarketplacePluginCard
      ...
      installed={makeItem({ source: 'factory' })}
      installState={{ status: 'up_to_date', installedVersion: '1.2.3' }}
      onInstall={onInstall}
    />
  )
  expect(screen.getByText('Updates with the system')).toBeInTheDocument()
  expect(
...
test.each(['override', 'override_over_factory'] as const)(
...
test('marketplace entry for uninstalled plugin offers installation', async () => {
  const user = userEvent.setup()
  const onInstall = vi.fn()
  render(
    <MarketplacePluginCard
  ...
  await user.click(screen.getByRole('button', { name: 'Install' }))
  expect(onInstall).toHaveBeenCalledOnce()
  expect(screen.queryByText('Updates with the system')).not.toBeInTheDocument()
})
