...
export function evalExprLocally(
  ...
  const result = evaluateBillingExpression(exprStr, {
    ...context,
    imageCount: extraTokenValues.imageCount ?? context?.imageCount,
    tokens: {
...
export function buildEstimatorTokens(
  ...
  return {
    ...
    len:
      (promptTokens || 0) +
      (extraTokenValues.cacheReadTokens || 0) +
      (extraTokenValues.imageCacheTokens || 0) +
      (extraTokenValues.imageTokens || 0) +
      (extraTokenValues.audioInputTokens || 0) +
      (extraTokenValues.cacheCreateTokens || 0) +
      (extraTokenValues.cacheCreate1hTokens || 0),
    ...Object.fromEntries(
      ESTIMATOR_VARS.map((field) => [
        field.var,
        extraTokenValues[field.stateKey] ?? 0,
      ])
    ),
  }
}
...
export function exprUsesExtraVars(exprStr: string): boolean {
  ...
  if (compiled.status === 'ready') {
    return (
      ESTIMATOR_VARS.some((field) => compiled.variables.has(field.var)) ||
...
export function exprUsesImageCount(exprStr: string): boolean {
  ...
  const compiled = compileBillingExpression(exprStr)
  if (compiled.status === 'ready') {
    return compiled.variables.has('image_count')
  }
