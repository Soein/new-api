...
func securityEnrollmentRequest(method, path, body, proof string, identity service.AuthIdentity, handler gin.HandlerFunc) *httptest.ResponseRecorder {
	...
	if generation, err := model.GetUserSessionGeneration(identity.UserID); err == nil {
...
func TestSecurityEnrollmentAccessTokenRequiresProofBeforeMutation(t *testing.T) {
	...
	require.NoError(t, model.UpdateUserAccessToken(user.Id, user.SessionGeneration, "existing-system-token"))
...
func TestSecurityEnrollmentAccessTokenLifecycleConsumesProofs(t *testing.T) {
	user, identity := setupSecurityEnrollmentTest(t)
	require.NoError(t, model.UpdateUserAccessToken(user.Id, user.SessionGeneration, "previous-token"))
...
func TestSecurityEnrollmentAccessTokenRejectsInvalidProofs(t *testing.T) {
	user, identity := setupSecurityEnrollmentTest(t)
	require.NoError(t, model.UpdateUserAccessToken(user.Id, user.SessionGeneration, "unchanged-token"))
	for _, endpoint := range []struct {
		method, scope string
		handler       gin.HandlerFunc
	}{
