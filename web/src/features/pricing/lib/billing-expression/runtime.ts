...
class BillingRuntime {
  ...
  readonly matchedRules: MatchedRequestRule[] = []
  matchedTier = ''
  billingUnit: 'token' | 'request' = 'token'
  fixedPrice?: number
  imageCount?: number
  private namedRuleIndex = 0
  ...
  private getImageCount(): number {
    const raw = this.context.imageCount
    if (raw === undefined) {
      return this.compiled.version === 2 ? 0 : 1
    }
    if (
      typeof raw !== 'number' ||
      !Number.isFinite(raw) ||
      ...
      throw new BillingExpressionError({
        code: 'number',
        detail:
          this.compiled.version === 2
            ? 'image_count must be an integer between 0 and 128'
            : 'image_count must be an integer between 1 and 128',
      })
    }
    if (this.compiled.version === 2) {
      if (raw < 0 || raw > 128) {
        throw new BillingExpressionError({
          code: 'number',
          detail: 'image_count must be between 0 and 128',
        })
      }
    } else {
      if (raw < 1 || raw > 128) {
  ...
  private evaluateNode(node: ExpressionNode): RuntimeValue {
    ...
    if (node.kind === 'variable') {
      if (node.name === 'image_count') {
        const value = this.getImageCount()
        if (this.compiled.version === 2) {
  ...
  private call(node: Extract<ExpressionNode, { kind: 'call' }>): RuntimeValue {
    ...
    switch (node.name) {
      ...
      case 'per_image': {
        const price = this.numeric(args[0]).value
        if (price < 0 || !Number.isFinite(price)) {
          throw new BillingExpressionError({
            code: 'number',
            detail: 'per_image price must be a finite non-negative number',
            position: args[0].start,
          })
        }
        const count = this.getImageCount()
        this.imageCount = count
        return { value: price * count * 1_000_000 }
      }
      case 'rule': {
        const name = this.string(args[0]).trim().slice(0, 256)
        const matched = this.boolean(args[1])
        const multiplier = this.numeric(args[2]).value
        if (multiplier <= 0 || !Number.isFinite(multiplier)) {
          throw new BillingExpressionError({
            code: 'number',
            detail: 'rule multiplier must be a finite positive number',
            position: args[2].start,
          })
        }
        const currentRuleIndex = this.namedRuleIndex++
        const ruleIndex = this.compiled.requestRules.findIndex(
          (candidate) => candidate.node === node
        )
        if (ruleIndex >= 0) {
          this.requestRules[ruleIndex].matched = matched
        }
        if (matched) {
  ...
  try {
    ...
    if (context.imageCount !== undefined) {
      if (
        ...
        !Number.isInteger(context.imageCount)
      ) {
        throw new BillingExpressionError({
          code: 'number',
          detail:
      ...
      if (compiled.version === 2) {
        if (context.imageCount < 0 || context.imageCount > 128) {
          throw new BillingExpressionError({
            code: 'number',
            detail: 'image_count must be between 0 and 128',
          })
        ...
        if (context.imageCount < 1 || context.imageCount > 128) {
          throw new BillingExpressionError({
            code: 'number',
            detail: 'image_count must be between 1 and 128',
          })
    ...
    const hasImageCount =
      ...
      compiled.functions.has('per_image') ||
      runtime.imageCount !== undefined ||
      context.imageCount !== undefined
    let finalImageCount = compiled.version === 2 ? 0 : 1
    if (runtime.imageCount !== undefined) {
      finalImageCount = runtime.imageCount
    } else if (context.imageCount !== undefined) {
      finalImageCount = context.imageCount
    }
    return {
      status: 'success',
      cost: result,
      billingUnit: runtime.billingUnit,
      ...(hasImageCount ? { imageCount: finalImageCount } : {}),
      ...
      ...(compiled.functions.has('rule') || runtime.matchedRules.length > 0
        ? {
            matchedRules: runtime.matchedRules,
            matchedRequestRules: runtime.matchedRules,
          }
        : {}),
    }
  } catch (error) {
