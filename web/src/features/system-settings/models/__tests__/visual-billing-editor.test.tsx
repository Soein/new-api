...
test.each(['hour("UTC") >= 9 ? max(p * 2, 100) : tier("off", p * 1)'])(
...
test('supports visual editing for v2 single tier expression and preserves v2 version prefix', () => {
  const source = 'v2:tier("base", p * 3 + c * 9)'
  const onBillingExprChange = vi.fn()
  render(
    <TieredPricingEditor
      billingExpr={source}
  ...
  const tier = within(screen.getByRole('group', { name: 'Pricing tier base' }))
  const expand = tier.queryByRole('button', {
    name: 'Edit pricing rule base',
  })
  if (expand && expand.getAttribute('aria-expanded') === 'false') {
    fireEvent.click(expand)
  }

  expect(tier.getByRole('textbox', { name: 'Input price' })).toHaveValue('3')
  expect(tier.getByRole('textbox', { name: 'Output price' })).toHaveValue('9')
  ...
  assert(lastCall)
  const generated = lastCall[0]
  expect(generated).toContain('v2:')
  expect(generated).toContain('tier("base"')
  expect(generated).toContain('p * 4')
  expect(generated).toContain('c * 9')
  expect(onBillingExprChange).toHaveBeenLastCalledWith(
    'v2:tier("base", p * 4 + c * 9)'
  )
  expect(
    evaluateBillingExpression(generated, {
      tokens: { p: 100, c: 10 },
    })
  ).toMatchObject({ status: 'success', matchedTier: 'base', cost: 490 })
})
