...
function mapTokenTier(
  ...
  return {
    ...
    ...(tier.billingUnit === 'request'
      ...
      : {}),
    ...(tier.perImagePrice !== undefined
      ? { perImagePrice: tier.perImagePrice }
      : {}),
...
export function stripExprVersion(exprStr: string): {
  version: number
  body: string
} {
...
export function parseTiersFromExpr(exprStr: string): ParsedTier[] {
  ...
  if (compiled.status !== 'ready') {
    try {
      ...
      const perImageMatch = body.match(
        /^\s*per_image\(\s*([\d.eE+-]+)\s*\)\s*$/
      )
      if (perImageMatch) {
        const perImagePrice = Number(perImageMatch[1])
        if (Number.isFinite(perImagePrice) && perImagePrice >= 0) {
          return [{ label: 'base', conditions: [], perImagePrice }]
        }
      }
    } catch {
      // ignore
...
export function splitBillingExprAndRequestRules(expr: string): {
  ...
  return {
    billingExpr:
      version > 1
        ? `v${version}:${baseParts.map(unwrapOuterParens).join(' * ')}`
        : baseParts.map(unwrapOuterParens).join(' * '),
