...
// taskAdjustFunding 调整任务的资金来源（钱包或订阅），delta > 0 表示任务完成后的补扣，
// delta < 0 表示退还。
func taskAdjustFunding(task *model.Task, delta int) error {
	...
	if delta > 0 {
		return model.DecreaseUserQuotaForSettlement(task.UserId, delta)
	}
...
func appendTaskBillingUsageSchema(task *model.Task, other *model.LogOther) {
	...
	other.SetPublic("billing_usage_schema", schema)
}
