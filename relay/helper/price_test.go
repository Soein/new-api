...
func TestModelPriceHelperFixedPricePreConsumesResponsesImageGenerationTool(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	overflowModelPrice := float64(common.MaxQuota-1000) / common.QuotaPerUnit / 0.1
	modelPrices, err := common.Marshal(map[string]float64{
		"responses-fixed-price-image-tool-model":          0.02,
		"responses-fixed-price-image-tool-overflow-model": overflowModelPrice,
	})
	require.NoError(t, err)
	...
	cases := []struct {
		...
		{
			name:     "adds tool reserve after fixed model price",
			model:    "responses-fixed-price-image-tool-model",
			expected: expectedModelQuota + expectedToolQuota,
		},
		{
			name:         "rejects combined reserve overflow",
			model:        "responses-fixed-price-image-tool-overflow-model",
			wantOverflow: true,
		},
	}
