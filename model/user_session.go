...
type UserSession struct {
	...
	UserGeneration      string `json:"-" gorm:"column:user_generation;type:char(32)"`
...
func (session *UserSession) cacheEntry() *userSessionCacheEntry {
	return &userSessionCacheEntry{
		...
		UserGeneration:  session.UserGeneration,
...
func (entry *userSessionCacheEntry) session() *UserSession {
	return &UserSession{
		SID:             entry.SID,
		UserID:          entry.UserID,
		UserGeneration:  entry.UserGeneration,
...
func CreateUserSession(session *UserSession) error {
	...
	if session.UserGeneration == "" {
		generation, err := GetUserSessionGeneration(session.UserID)
		if err != nil {
			return err
		}
