...
func TryTieredSettle(relayInfo *relaycommon.RelayInfo, params billingexpr.TokenParams) (ok bool, quota int, result *billingexpr.TieredResult) {
	...
	resolvedCount := relayInfo.GetImageBillingCount()
	if resolvedCount > 0 {
		params.ImageCount = float64(resolvedCount)
		requestInput.ImageCount = &resolvedCount
	} else if relayInfo.BillingImageCount != nil {
		params.ImageCount = float64(*relayInfo.BillingImageCount)
		requestInput.ImageCount = relayInfo.BillingImageCount
	} else if snap.EstimatedImageCount != nil {
		params.ImageCount = float64(*snap.EstimatedImageCount)
		requestInput.ImageCount = snap.EstimatedImageCount
	} else if snap.ImageCount > 0 {
		params.ImageCount = float64(snap.ImageCount)
		count := snap.ImageCount
