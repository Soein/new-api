...
export function ChannelMutateDrawer({
  ...
  const responseTimeThresholdFields = (
    <FormField
      control={form.control}
      name='response_time_threshold_sec'
      render={({ field }) => (
        <FormItem>
          <FormLabel>{t('Response Time Threshold (seconds)')}</FormLabel>
          <FormControl>
            <Input
              type='number'
              min={0}
          ...
          <FormDescription>
            {t(
              'Overrides the global threshold for scheduled-test response time and the production first-token breaker, 0 means use the global setting'
            )}
          </FormDescription>
          <FormMessage />
        </FormItem>
