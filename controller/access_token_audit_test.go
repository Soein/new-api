...
func TestAccessTokenLifecycleAndLateRequests(t *testing.T) {
	...
	router.POST("/rotate-in-flight", middleware.UserAuth(), func(c *gin.Context) {
		require.NoError(t, model.UpdateUserAccessToken(user.Id, user.SessionGeneration, "new-token"))
	...
	_, err = model.RevokeUserAccessToken(user.Id, user.SessionGeneration)
	require.NoError(t, err)
	assert.Equal(t, 401, auditRequest(router, "GET", "/api/user/token/status", "new-token").Code)
	ref, err := model.RevokeUserAccessToken(user.Id, user.SessionGeneration)
	require.NoError(t, err)
...
func TestAuditIsolationVisibilityAndFailureContracts(t *testing.T) {
	...
	require.Error(t, model.UpdateUserAccessToken(user.Id, user.SessionGeneration, "replacement"))
	_, err = model.RevokeUserAccessToken(user.Id, user.SessionGeneration)
	require.Error(t, err)
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, upgrade := range []bool{false, true} {
				t.Run(fmt.Sprintf("upgrade=%v", upgrade), func(t *testing.T) {
					...
					require.NoError(t, model.DB.First(&tokenOwner, 1).Error)
					require.NoError(t, model.UpdateUserAccessToken(tokenOwner.Id, tokenOwner.SessionGeneration, "matrix-token"))
					...
					_, err = model.RevokeUserAccessToken(tokenOwner.Id, tokenOwner.SessionGeneration)
					require.NoError(t, err)
					_, err = model.RevokeUserAccessToken(tokenOwner.Id, tokenOwner.SessionGeneration)
					require.NoError(t, err)
	...
	for _, tc := range []struct{ kind, env string }{{"mysql", "AUDIT_MYSQL_DSN"}, {"postgres", "AUDIT_POSTGRES_DSN"}, {"clickhouse", "AUDIT_CLICKHOUSE_DSN"}} {
		for _, upgrade := range []bool{false, true} {
			t.Run(fmt.Sprintf("%s/upgrade=%v", tc.kind, upgrade), func(t *testing.T) {
				...
				var tokenOwner model.User
				require.NoError(t, model.DB.First(&tokenOwner, 1).Error)
				require.NoError(t, model.UpdateUserAccessToken(tokenOwner.Id, tokenOwner.SessionGeneration, "independent-pat"))
