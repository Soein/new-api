...
function formatBreakdownPrice(
  ...
  if (field.unit === 'image') {
    if (options?.isTableCell) return amount
    return `${amount}/${t('image')}`
  }
...
export function DynamicPricingBreakdown({
  ...
  const { tiers, ruleGroups } = useMemo(() => {
    ...
    let parsedRules = (expressionRules || []).map((group) => {
      if (group.name) {
    ...
    if (parsedRules.length === 0 && requestRules == null) {
  ...
  }, [expr, matchedRequestRules, requestRules, usageSchema])
  ...
  const visiblePriceFields: BreakdownPriceField[] = (() => {
    ...
    const fields: BreakdownPriceField[] = [
      ...BILLING_PRICING_VARS,
      PER_IMAGE_BILLING_VAR,
    ]
      .filter((variable) => {
      ...
      .map((variable, index) => ({
        ...
        unit: (variable.unit ?? 'token') as 'token' | 'image',
        value: (tier: BreakdownTier) =>
          isTaskBreakdownTier(tier)
            ? 0
