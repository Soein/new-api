...
func TestGetAndValidOpenAIImageRequestNBounds(t *testing.T) {
	...
	t.Run("nested parameters n does not override top-level n", func(t *testing.T) {
		c := newJSONContext(t, `{"model":"wan-image","prompt":"a cat","parameters":{"n":3}}`)
		req, err := GetAndValidOpenAIImageRequest(c, relayconstant.RelayModeImagesGenerations)
		require.NoError(t, err)
		require.Equal(t, uint(1), *req.N)
		require.NotNil(t, req.BillingParameters)
		require.NotNil(t, req.BillingParameters.N)
		require.Equal(t, uint(3), *req.BillingParameters.N)
