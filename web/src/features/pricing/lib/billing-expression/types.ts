...
export const V1_BILLING_FUNCTIONS: Readonly<Record<string, number>> = {
  tier: 2,
  fixed: 1,
  param: 1,
  header: 1,
...
export const V2_BILLING_FUNCTIONS: Readonly<Record<string, number>> = {
...
export type MatchedRequestRule = {
  index: number
  name: string
  multiplier: number
}

export type ExpressionVersion = 1 | 2
...
export type BillingEvaluationResult =
  ...
  | {
      ...
      matchedRules?: MatchedRequestRule[]
      matchedRequestRules?: MatchedRequestRule[]
    }
...
export function expressionDependencies(node: ExpressionNode): {
  ...
  visitExpression(node, (part) => {
    ...
    if (part.kind === 'call') {
      ...
      if (part.name === 'per_image') {
        variables.add('image_count')
      }
    }
  })
