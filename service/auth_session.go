...
func ValidateLoginSession(identity AuthIdentity) (*model.UserSession, *model.UserBase, error) {
	...
	if user.Status != common.UserStatusEnabled || user.AuthVersion != identity.UserAuthVersion || session.UserGeneration == "" || session.UserGeneration != user.SessionGeneration {
		return nil, nil, ErrLoginSessionRevoked
	}
