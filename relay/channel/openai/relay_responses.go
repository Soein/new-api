...
func validateAndParseResponsesUsage(rawUsage json.RawMessage) (*dto.Usage, bool) {
	...
	if raw, ok := rawMap["output_tokens_details"]; ok {
		if !validateKnownDetailObject(raw, outputDetails) {
			return nil, false
		}
	}
	...
	if hasBothCoreCounters {
		...
		if decodedUsage.InputTokensDetails != nil {
			inputDetails := *decodedUsage.InputTokensDetails
			normalized.InputTokensDetails = &inputDetails
		}
		if decodedUsage.OutputTokensDetails != nil {
			normalized.CompletionTokenDetails = *decodedUsage.OutputTokensDetails
		}
		normalized.PromptCacheHitTokens = decodedUsage.PromptCacheHitTokens
		if normalized.CompletionTokenDetails != (dto.OutputTokenDetails{}) {
			outputDetails := normalized.CompletionTokenDetails
			normalized.OutputTokensDetails = &outputDetails
		}
...
func OaiResponsesStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	helper.StreamScannerHandler(c, resp, info, func(data string, sr *helper.StreamResult) {
		...
		if len(envelope.Response) > 0 && string(envelope.Response) != "null" {
			...
			data = string(rewriteSGLangResponsesCreatedAt(info, []byte(data), "response.created_at", respEnv.CreatedAt))
		}
...
func rewriteSGLangResponsesCreatedAt(info *relaycommon.RelayInfo, payload []byte, path string, rawCreatedAt json.RawMessage) []byte {
	...
	if len(rawCreatedAt) == 0 {
		...
		rawCreatedAt = json.RawMessage(res.Raw)
	}
	if isAbsentOrNull(rawCreatedAt) {
		return payload
	}
	var parsedCreatedAt dto.IntValue
	if err := common.Unmarshal(rawCreatedAt, &parsedCreatedAt); err != nil {
		return payload
	}
	patched, err := sjson.SetBytes(payload, path, int(parsedCreatedAt))
	if err != nil {
		return payload
	}
	return patched
