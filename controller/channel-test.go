...
func testChannelForHealthCheck(ctx context.Context, channel *model.Channel, testUserID int, allowDisable bool, disableThreshold int64) channelTestSummary {
	...
	if common.AutomaticDisableChannelEnabled && !shouldBanChannel {
		channelDisableThreshold := disableThreshold
		if setting := channel.GetSetting(); setting.ResponseTimeThresholdSec > 0 {
			channelDisableThreshold = int64(setting.ResponseTimeThresholdSec * 1000)
		}
		if milliseconds > channelDisableThreshold {
			err := fmt.Errorf("响应时间 %.2fs 超过阈值 %.2fs", float64(milliseconds)/1000.0, float64(channelDisableThreshold)/1000.0)
