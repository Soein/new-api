...
func ModelPriceHelper(c *gin.Context, info *relaycommon.RelayInfo, promptTokens int, meta *types.TokenCountMeta) (hosttypes.PriceData, error) {
	...
	if toolPreConsumedQuota > 0 {
		quota, err := common.QuotaFromFloatStrict(float64(priceData.QuotaToPreConsume) + float64(toolPreConsumedQuota))
		if err != nil {
			return hosttypes.PriceData{}, err
		}
		priceData.QuotaToPreConsume = quota
...
func modelPriceHelperTiered(c *gin.Context, info *relaycommon.RelayInfo, billingModelName string, promptTokens int, meta *types.TokenCountMeta, groupRatioInfo hosttypes.GroupRatioInfo) (hosttypes.PriceData, error) {
	...
	if vars := billingexpr.UsedVarsByHash(exprStr, exprHash); vars["image_count"] || vars["per_image"] {
