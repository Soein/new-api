...
func setupResponsesWSRequestTest(t *testing.T) (*model.User, *model.Token) {
	...
	require.NoError(t, db.AutoMigrate(&model.User{}, &model.Token{}, &model.UserQuotaDebt{}))
...
func TestResponsesWebSocketFreezesPricingPerCreateAcrossDynamicUpdates(t *testing.T) {
	var savedToolPrices string
	require.NoError(t, config.GlobalConfig.SaveToDB(func(key, value string) error {
		if key == operation_setting.ToolPriceOptionKey {
			savedToolPrices = value
		}
		return nil
	}))
	t.Cleanup(func() {
		operation_setting.LoadToolPricesFromJSONString(savedToolPrices)
	})
	...
	fixture := newResponsesWSBillingTest(t, `tier("base", p * 2)`, func(ws *websocket.Conn, r *http.Request) {
		for index := 1; ; index++ {
			_, data, err := ws.ReadMessage()
			if err != nil {
				return
			...
			if index == 1 {
				assert.Empty(t, event.PreviousResponseID)
				...
				assert.Equal(t, "resp_1", event.PreviousResponseID)
			}
			terminal := fmt.Sprintf(`{"type":"response.completed","response":{"id":"resp_%d","status":"completed","model":"ws-billing","output":[{"type":"image_generation_call","id":"img_%d","status":"completed","result":"base64-%d"}],"usage":{"input_tokens":100,"output_tokens":0,"total_tokens":100}}}`, index, index, index)
			if !assert.NoError(t, ws.WriteMessage(websocket.TextMessage, []byte(terminal))) {
				return
			}
		}
	...
	require.NoError(t, fixture.client.WriteMessage(websocket.TextMessage, []byte(`{"type":"response.create","model":"ws-billing","input":"first","tools":[{"type":"image_generation"}]}`)))

	select {
	case <-firstReceived:
	case <-time.After(3 * time.Second):
		t.Fatal("upstream did not receive the initial create")
	}

	expressions, err := common.Marshal(map[string]string{"ws-billing": `tier("base", p * 4)`})
	require.NoError(t, err)
	require.NoError(t, config.GlobalConfig.LoadFromDB(map[string]string{
		"billing_setting.billing_expr": string(expressions),
	}))
	operation_setting.LoadToolPricesFromJSONString(`{"image_generation":2}`)
	release()

	firstEvent := readResponsesWSTestEvent(t, fixture.client)
	require.Equal(t, "response.completed", firstEvent["type"])
	firstResponse, ok := firstEvent["response"].(map[string]any)
	require.True(t, ok)
	firstResponseID, _ := firstResponse["id"].(string)
	assert.Equal(t, "resp_1", firstResponseID)

	secondPayload := fmt.Sprintf(`{"type":"response.create","model":"ws-billing","input":"second","previous_response_id":"%s","tools":[{"type":"image_generation"}]}`, firstResponseID)
	require.NoError(t, fixture.client.WriteMessage(websocket.TextMessage, []byte(secondPayload)))

	secondEvent := readResponsesWSTestEvent(t, fixture.client)
	require.Equal(t, "response.completed", secondEvent["type"])
	secondResponse, ok := secondEvent["response"].(map[string]any)
	require.True(t, ok)
	assert.Equal(t, "resp_2", secondResponse["id"])

	fixture.closeAndWait(t)
	assertResponsesWSAccounting(t, fixture, []int{600, 1200})
	assert.Equal(t, int32(1), fixture.connections.Load())
}
