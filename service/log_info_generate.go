...
func InjectTieredBillingInfo(other *model.LogOther, relayInfo *relaycommon.RelayInfo, result *billingexpr.TieredResult) {
	...
	if imageCount := relayInfo.GetImageBillingCount(); imageCount > 0 {
		other.SetPublic("image_count", imageCount)
	}
	if result != nil {
		other.SetPublic("matched_tier", result.MatchedTier)
		if len(result.MatchedRules) > 0 {
			other.SetPublic("matched_request_rules", result.MatchedRules)
		}
