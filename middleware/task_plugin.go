...
func PrepareTaskPluginEndpoint() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		admitted := make([]pluginruntime.ProtocolBinding, 0, len(candidates))
		for _, candidate := range candidates {
			if candidate.Plugin != nil && service.TaskPluginAdmittedForNewRequest(candidate.Plugin) {
		...
		pinned.Candidates = admitted
		pinned.Plugin = admitted[0].Plugin
		pinned.Protocol = admitted[0].Protocol
		pinned.Operation = admitted[0].Operation
